...
import { CreateNewProduct, FetchProductDetails, UpdateProductData } from '../../Redux/Slicers/Products';
...
const UpdateProduct = () => {
    ...
    // handling product Id to update
    const [pId, setpId] = useState('')

    const fetchproductData = async () => {
        await dispatch(FetchProductDetails(pId));
    }

    //handling product data
    const { isLoading, isError, Errmsg, productdetails } = useSelector(state => state.Products)
    ...
    const submitCreateForm = async (e) => {
        ...
        console.log('update data list is ', updatelist)
        console.log('ProductId', pId)
        await dispatch(UpdateProductData({ 'prductdata': productData, pId }));
    ...
    useEffect(() => {
        ...
        if (Object.keys(productdetails).length > 0) {
            let details = productdetails.response
            setName(details.name)
            setDescription(details.description)
            setCategories([...details.category])
            setPrice(details.price)
            setRatings(details.ratings)
            setStock(details.stock)
            setImages([...details.image])
            setImagePreviews([...details.image])
        }
    ...
    return (
        <div className="max-w-4xl max-sm:w-full mx-auto p-6">
            ...
            <div className='w-full px-4 py-2'>
                <input className="p-3 w-full outline-none border rounded-xl pl-12" type="text" name='productid' value={pId} onChange={(e) => setpId(e.target.value)} placeholder='Enter Product Id to Edit' />
                <button onClick={() => fetchproductData()} className='w-full my-2 bg-blue-600 text-white p-2 rounded-lg mt-4 hover:bg-blue-900 transition-all'>Fetch Product</button>
            </div>
            {
